...
- モデルテストでは、有効サーベイ判定を確認する。
- モデルテストでは、回答済み判定を確認する。
- モデルテストでは、スコア範囲を確認する。
- モデルテストでは、ユニーク制約を確認する。
- 統合テストでは、ログイン済みユーザーがホームを見ることを確認する。
- 統合テストでは、未回答サーベイに回答できることを確認する。
- 統合テストでは、回答済み・期限切れ・対象外では表示されないことを確認する。
